...
# -----------------------------------------------------------------------
# Multi-track clones: same codec/fps/container as base entries but with
# N independent audio tracks carrying language tags. We do NOT cross the
# codec/fps/container grid with track count — these are the only multi-track
# clips, using the same combos as base entries.
#
# Each row: NAME CODEC FPS CONTAINER FRAMES AUDIO_TRACKS DEFAULT_TRACK LANG1 LANG2 ...
# -----------------------------------------------------------------------
MULTI_TRACK_MATRIX=(
...
# Write the manifests the coverage tests consume, derived from MATRIX and
# MULTI_TRACK_MATRIX so the generator stays the single source of truth.
# matrix.list is the whitespace-separated file the headless test binaries parse;
# matrix.json is a richer, human-readable mirror. Both list the full matrix
# regardless of which clips were generated — a listed-but-missing clip is
# skipped (with a WARN) by the tests, never a failure.
#
# Multi-track entries carry extra columns after audio_rate:
#   audio_tracks  default_track  lang1  lang2 ...
write_manifests() {
    local list="${OUTPUT_DIR}/matrix.list"
    local json="${OUTPUT_DIR}/matrix.json"
    local row name codec fps container frames lang_list

    {
        ...
        # Multi-track entries: extra columns for track count, default index, langs.
        for row in "${MULTI_TRACK_MATRIX[@]}"; do
            # shellcheck disable=SC2086
            set -- ${row}
...

    {
        ...
        echo "  \"height\": ${HEIGHT},"
        echo "  \"clips\": ["
        local n=$(( ${#MATRIX[@]} + ${#MULTI_TRACK_MATRIX[@]} )) i=0 comma first=1
        for row in "${MATRIX[@]}"; do
            # shellcheck disable=SC2086
            set -- ${row}
...
gen_multi_track_clip() {
    ...
    for ((track=0; track<num_tracks; track++)); do
        ...
        # then upmixed mono -> STEREO to match the manifest's audio_channels=2.
        for ((i=0; i<frames; i++)); do
            freq=$(( track_base + 200 * i ))
            audio_filter+="sine=frequency=${freq}:sample_rate=${SAMPLE_RATE}:duration=${frame_dur}[t${track}f${i}];"
        done
        for ((i=0; i<frames; i++)); do
            audio_filter+="[t${track}f${i}]"
        done
        audio_filter+="concat=n=${frames}:v=0:a=1[${label}_mono];"
        audio_filter+="[${label}_mono]pan=stereo|c0=c0|c1=c0[${label}]"
