...
//   - a MetalSurfaceImporter that turns a decoder CVPixelBuffer into two RD
...
public:
	...
	// `frame.native_handle` must be a CVPixelBufferRef (macOS). Returns true if
	// a GPU pass ran. The frame's own release() is parked in the retire-ring.
...
private:
	...
	MetalSurfaceImporter importer_;
